...
void CC1101::waitForState(State state) {
  while (getState() != state) {
    // Serial.printf("waiting for state: %d, currentState: %d, task: %s", state, getState(), pcTaskGetName(NULL));
    // Serial.println();
  };
};
...
void CC1101::setState(State state) {
  ...
  switch (state) {
    ...
    case STATE_RX: 
      ...
      else if (currentState != (STATE_CALIB || STATE_SETTLING)) 
        // strobe(CC1101_REG_RX);
...
uint8_t CC1101::readRegField(byte addr, byte lo, byte hi) {
  // return bus.readField(addr | CC1101_READ, lo, hi);
